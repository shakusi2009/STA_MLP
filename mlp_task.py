    ...
    try:
        ...
        if hasattr(resp, 'items'):
            for job in resp.items:
                job_info = {
                    '任务创建人': getattr(job, 'created_by', None),
                    '任务创建时间': time_utils.time_convert(getattr(job, 'create_time', None)),
                    '最大运行时常(小时)': getattr(getattr(job, 'resource_config', None), 'max_runtime_seconds', None)/3600 if hasattr(job, 'resource_config') else None,
                    '资源信息': []
                }
